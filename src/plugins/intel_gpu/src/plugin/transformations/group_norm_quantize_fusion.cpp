...
GroupNormQuantizeFusion::GroupNormQuantizeFusion() {
    ...
    auto dyn_scale_m = wrap_type<op::DynamicScale>({ swish_m });
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        OPENVINO_ASSERT(pattern_map.count(dyn_scale_m));
        ...
        auto dyn_scale = std::dynamic_pointer_cast<op::DynamicScale>(pattern_map.at(dyn_scale_m).get_node_shared_ptr());
        if (!dyn_scale || transformation_callback(dyn_scale))
            return false;
        ...
        ov::copy_runtime_info({group_norm, swish, dyn_scale}, group_norm_quant);
        ov::replace_node(dyn_scale, group_norm_quant);
 
        return true;
    };

    auto m = std::make_shared<ov::pass::pattern::Matcher>(dyn_scale_m, "GroupNormQuantizeFusion");
