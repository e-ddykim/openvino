...
GroupNormQuantizeKernelBase::MultiDispatchData GroupNormQuantizeKernelBfyx::SetDefault(const group_norm_quantize_params &params) const {
    ...
    if (!params.has_dynamic_tensors()) {
        ...
        dispatchData.stage_max.gws[0] = input.Feature().v;
        dispatchData.stage_max.gws[1] = input.Batch().v;
        dispatchData.stage_max.gws[2] = 1;

        dispatchData.stage_max.lws[0] = input.Feature().v;
...
void GroupNormQuantizeKernelBfyx::GetUpdateDispatchDataFunc(KernelData& kd) const {
    kd.update_dispatch_data_func = [this](const Params& params, KernelData& kd) {
        ...
        kd.kernels[4].params.workGroups.global = dispatchData.stage_max.gws;
        kd.kernels[4].params.workGroups.local = dispatchData.stage_max.lws;
        kd.kernels[4].skip_execution = KernelData::SkipKernelExecution(prim_params, 4);

        kd.kernels[5].params.workGroups.global = dispatchData.stage_final.gws;
        kd.kernels[5].params.workGroups.local = dispatchData.stage_final.lws;
        kd.kernels[5].skip_execution = KernelData::SkipKernelExecution(prim_params, 4);
...
KernelsData GroupNormQuantizeKernelBfyx::GetKernelsData(const Params &params) const {
    ...
    KernelData kd = KernelData::Default<group_norm_quantize_params>(params, 6);
    ...
    {
        // per-batch max stage
        auto cldnn_jit = GetJitConstants(prim_params, dispatchData.stage_max);
        cldnn_jit.AddConstant(MakeJitConstant("GROUP_NORM_MAX", 1));
        auto entry_point = GetEntryPoint(finalKernelName, prim_params.layerID, params, entry_part_id++);
        auto jit = CreateJit(finalKernelName, cldnn_jit, entry_point);
