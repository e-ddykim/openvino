...
namespace cldnn {
GPU_DEFINE_PRIMITIVE_TYPE_ID(group_norm_quantize)

layout group_norm_quantize_inst::calc_output_layout(group_norm_quantize_node const& node, kernel_impl_params const& impl_param) {
...
std::vector<layout> group_norm_quantize_inst::calc_output_layouts(group_norm_quantize_node const& /*node*/, const kernel_impl_params& impl_param) {
    auto desc = impl_param.typed_desc<group_norm_quantize>();
    auto input_node_layout = impl_param.get_non_padded_input_layout();
...
std::string group_norm_quantize_inst::to_string(group_norm_quantize_node const& node) {
    ...
    json_composite group_norm_quantize_info;
    group_norm_quantize_info.add("dimension", desc->num_groups);
    group_norm_quantize_info.add("epsilon", desc->epsilon);

    node_info->add("group_norm_quantize_info", group_norm_quantize_info);
    node_info->dump(primitive_description);

    return primitive_description.str();
}

group_norm_quantize_inst::typed_primitive_inst(network& network, group_norm_quantize_node const& node) : parent(network, node) {}
